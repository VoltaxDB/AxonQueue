...
namespace taskqueuex
{
    ...
    {
        size_t index = storage_pool_index_.fetch_add(1, std::memory_order_relaxed) %
                       STORAGE_POOL_SIZE;
    ...
    nlohmann::json CLICommands::enqueue(const nlohmann::json &params)
    {
        try
        {
            ...
            return {{"status", "success"}, {"task_id", task->getId()}};
        }
    ...
    nlohmann::json CLICommands::setMode(const nlohmann::json &params)
    {
        try
        {
            ...
            if (mode_str == "FIFO")
                mode = QueueMode::FIFO;
            else if (mode_str == "LIFO")
                mode = QueueMode::LIFO;
            else if (mode_str == "ROUND_ROBIN")
                mode = QueueMode::ROUND_ROBIN;
            else if (mode_str == "PRIORITY")
                mode = QueueMode::PRIORITY;
            else
                throw std::invalid_argument("Invalid queue mode");
    ...
    nlohmann::json CLICommands::graph(const nlohmann::json &params)
    {
        try
        {
            ...
            return {{"status", "success"}, {"graph", dot}};
        }
        catch (const std::exception &e)
        {
            return {{"status", "error"}, {"message", e.what()}};
    ...
    nlohmann::json CLICommands::ready(const nlohmann::json & /*params*/)
    {
        try
        {
            ...
            {
                if (task->getStatus() == TaskStatus::QUEUED && task->areDependenciesMet() &&
            ...
            return {{"status", "success"}, {"tasks", ready_tasks}};
        }
        catch (const std::exception &e)
        {
            return {{"status", "error"}, {"message", e.what()}};
    ...
    void CLICommands::logEvent(Task::TaskId       id,
