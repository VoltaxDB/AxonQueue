...
namespace taskqueuex
{
    ...
    void TCPServer::Session::handleMessage(const std::string &message)
    {
        ...
        try
        {
            ...
            if (request.contains("command") && request["command"].is_string())
            {
                ...
                server_.queueWork(
                    [this, command, params, start_time]()
                    {
    ...
    TCPServer::TCPServer(boost::asio::io_context      &io_context,
        ...
        : address_(address), port_(port), io_context_(io_context), acceptor_(io_context),
          queue_manager_(queue_manager), storage_(storage)
    {
    ...
    void TCPServer::start()
    {
        ...
        std::thread([this]() { monitorConnections(); }).detach();
    }
    ...
    json TCPServer::executeCommand(const std::string &name, const json &params)
    {
        ...
        {
            if (name == "enqueue")
                return cli_commands_->enqueue(params);
            else if (name == "dequeue")
                return cli_commands_->dequeue(params);
            else if (name == "run")
                return cli_commands_->run(params);
            else if (name == "cancel")
                return cli_commands_->cancel(params);
            else if (name == "kill")
                return cli_commands_->kill(params);
            else if (name == "status")
                return cli_commands_->status(params);
            else if (name == "list")
                return cli_commands_->list(params);
            else if (name == "clear")
                return cli_commands_->clear(params);
            else if (name == "setMode")
                return cli_commands_->setMode(params);
            else if (name == "setConcurrency")
                return cli_commands_->setConcurrency(params);
            else if (name == "setRetry")
                return cli_commands_->setRetry(params);
            else if (name == "setDelay")
                return cli_commands_->setDelay(params);
            else if (name == "setSchedule")
                return cli_commands_->setSchedule(params);
            else if (name == "depend")
                return cli_commands_->depend(params);
            else if (name == "graph")
                return cli_commands_->graph(params);
            else if (name == "ready")
                return cli_commands_->ready(params);
            else if (name == "connect")
                return cli_commands_->connect(params);
            else if (name == "submit")
                return cli_commands_->submit(params);
            else if (name == "exportTask")
                return cli_commands_->exportTask(params);
            else if (name == "importTask")
                return cli_commands_->importTask(params);
        }
        catch (const std::exception &e)
        {
            return {{"status", "error"},
    ...
    std::unordered_map<std::string, double> TCPServer::getMetrics() const
    {
        ...
        metrics["avg_response_time_ms"] =
            (total_reqs > 0) ? static_cast<double>(total_response_time_ms_.load()) / total_reqs
                             : 0.0;
    ...
    {
        registerCommand("metrics", [this](const json & /*params*/) { return json(getMetrics()); });
    }
        ...
        while (workers_running_)
        {
...

            {
                ...
                work_condition_.wait(
                    lock, [this]() { return !workers_running_ || !work_queue_.empty(); });
