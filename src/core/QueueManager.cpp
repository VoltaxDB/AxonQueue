...
QueueManager::QueueManager(QueueMode initial_mode)
    : mode_(initial_mode) {
...
QueueManager::TaskPtr QueueManager::dequeueFIFO() {
    for (const auto& group : group_order_) {
        ...
        if (!queue.empty()) {
            ...
            total_wait_time_ += std::chrono::duration_cast<std::chrono::milliseconds>(wait_time).count();
...
bool QueueManager::empty() const {
    if (mode_ == QueueMode::PRIORITY) {
        return std::all_of(priority_queues_.begin(), priority_queues_.end(),
                          [](const TaskQueue& q) { return q.empty(); });
    }
    return std::all_of(group_queues_.begin(), group_queues_.end(),
