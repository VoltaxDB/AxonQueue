...
int main(int argc, char *argv[])
{
    try
    {
        ...
        desc.add_options()("help", "produce help message")(
            ...
            "number of worker threads")("db-path",
                                        po::value<std::string>()->default_value("taskqueue.db"),
        ...
        auto storage = std::make_shared<taskqueuex::Storage>(vm["db-path"].as<std::string>());
        ...
        auto thread_pool = std::make_shared<taskqueuex::ThreadPool>(vm["threads"].as<size_t>());
        thread_pool->start();

        boost::asio::io_context io_context;

        auto server = std::make_shared<taskqueuex::TCPServer>(io_context,
        ...
        {
            server->enableTLS(vm["tls-cert"].as<std::string>(), vm["tls-key"].as<std::string>());
        }

        server->start();

        std::cout << "TaskQueueX server started on " << vm["host"].as<std::string>() << ":"
                  << vm["port"].as<uint16_t>() << "\n";
